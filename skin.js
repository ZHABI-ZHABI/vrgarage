...
function pano2vrSkin(player,base) {
	...
	this.addSkin=function() {
		...
		me._map.logicBlock_position = function() {
			...
			if (me._map.ggCurrentLogicStatePosition != newLogicStatePosition) {
				...
				me._map.style[domTransition]='right 0s, bottom 0s, ' + cssPrefix + 'transform 0s';
				if (me._map.ggCurrentLogicStatePosition == 0) {
					me._map.style.right='-50px';
					me._map.style.bottom='50px';
				}
				else {
					me._map.style.right='0px';
					me._map.style.bottom='50px';
				}
			}
		}
	...
	function SkinHotspotClass_hotspot_up(parentScope,hotspot) {
		...
		me._hotspot_up.onmouseover=function (e) {
			player.setActiveHotspot(me.hotspot);
			skin._marker_titler.style[domTransition]='none';
			skin._marker_titler.style.visibility=(Number(skin._marker_titler.style.opacity)>0||!skin._marker_titler.style.opacity)?'inherit':'hidden';
			skin._marker_titler.ggVisible=true;
			skin.hotspotProxyOver(me.hotspot.id, me.hotspot.url);
		}
		me._hotspot_up.onmouseout=function (e) {
			player.setActiveHotspot(null);
			skin._marker_titler.style[domTransition]='none';
			skin._marker_titler.style.visibility='hidden';
			skin._marker_titler.ggVisible=false;
			skin.hotspotProxyOut(me.hotspot.id, me.hotspot.url);
		}
	...
	player.addListener('configloaded', function(args) { me._map.logicBlock_position();me._map.logicBlock_scaling(); });
	me.skinTimerEvent();
};